...
from ddpo_pytorch.intrinsic_rewards import RND
from ddpo_pytorch.diffusers_patch.likelihood import ode_likelihood, get_prior_likelihood
import torch
import wandb
...
FLAGS = flags.FLAGS
config_flags.DEFINE_config_file("config", "config/explore_rnd.py", "Training configuration.")
...
def main(_):
    ...
    # (diffusion) load scheduler, tokenizer and models.
    ...
    if config.rnd_ir.use_rnd:
        rnd = RND(config.rnd_ir.in_ch, config.rnd_ir.feature_dim, config.rnd_ir.intrinsic_coef)
    ...
    if config.rnd_ir.use_rnd:
        rnd_pred = rnd.predictor_network
        rnd_target = rnd.random_target_network

        rnd_optimizer = torch.optim.Adam(
            rnd_pred.parameters(),
            lr=config.rnd_ir.learning_rate,
            betas=(config.rnd_ir.adam_beta1, config.rnd_ir.adam_beta2),
            weight_decay=config.rnd_ir.adam_weight_decay,
            eps=config.rnd_ir.adam_epsilon,
        )
    ...
    if config.rnd_ir.use_rnd:
        unet, rnd_pred, rnd_target, unet_optimizer, rnd_optimizer = accelerator.prepare(
            unet, rnd_pred, rnd_target, unet_optimizer, rnd_optimizer
        )
    else:
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            # sample
            with autocast():
                images, _, latents, log_probs, all_noise_preds = pipeline_with_logprob(
                    pipeline,
            ...
            if config.ll_ir.use_ll:
                ...
                prior_likelihood = get_prior_likelihood(prior_traj, sigma_max.item())
            ...
            accelerator.log(
                {
                    "images": [
                        wandb.Image(
                            ...
                            caption=f"{prompt:.25} | {reward:.2f} | {ir:.2f}",
                        )
                        for i, (prompt, reward, ir) in enumerate(
                            zip(prompts, rewards, irs)
        ...
        # if config.rnd_ir.use_rnd or config.ll_ir.use_ll:
        #     irs = accelerator.gather(samples["intrinsic_rewards"]).cpu().numpy()
        ...
        accelerator.log(
            {
                "reward": rewards,
                "intrinsic_rewards": irs,
                "epoch": epoch,
                "reward_mean": rewards.mean(),
                "reward_std": rewards.std(),
                "intrinsic_reward_mean": irs.mean(),
                "intrinsic_reward_std": irs.std(),
            },
            step=global_step,
        )
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, sample in tqdm(
                ...
                # for name, param in unet.named_parameters():
                #     if param.requires_grad:
                #         diff = (param.data - old_params[name]).abs().sum()
                #         print(f"{name} update diff = {diff.item()}")
                ...
                if config.rnd_ir.use_rnd:
                    ...
                    info["intrinsic_reward_loss"].append(rnd_loss)
                    loss += rnd_loss
                    ...
                    if config.rnd_ir.max_grad_norm is not None:
                        accelerator.clip_grad_norm_(
                            rnd.predictor_network.parameters(), config.rnd_ir.max_grad_norm
                        )
